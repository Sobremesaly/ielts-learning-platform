<template>
  <div>
    <el-container>
      ...
      <el-main>
        <div class="joinClass" :style="{backgroundImage: 'url('+course.coursePicture+')'}">
          <div class="classContent">
            <div>
              <img class="schoolPic" src="../../assets/school_png.png">
            </div>
            <p class="classTitle">{{ course.courseName }}</p>
            <p class="classTeacher">课程由{{ course.courseMajor }}系审核 授课老师:{{ course.teacherName }}</p>
            <p class="classTime">课程时间:{{
                course.courseStartTime | formatDate
              }}至{{ course.courseEndTime | formatDate }}</p>
            <p class="classPeople">已有{{ course.courseStudentNum }}人加入课程</p>
            <button class="joinNow" v-if="joinCourse === 'no'" @click="studentJoinCourse">加入课程</button>
            <button class="joinNow" v-else @click="studentGoStudy">去学习</button>
          </div>
        </div>
        <div class="classPlate">
          <div class="shortIntroduce">
            <p class="Title">课程介绍</p>
            <hr>
            <p class="context">{{ course.courseRemark }}</p>
          </div>
...
export default {
  ...
  methods: {
    async getCourseByNum() {
      let course = {
        courseNum: this.$store.state.student.courseNum,
      }
      let {data: res} = await this.$http.post("getCourseByNum", qs.stringify(course), {headers: {'Content-Type': 'application/x-www-form-urlencoded'}});
    ...
    async studentJoinCourse() {
      ...
      let {data: res} = await this.$http.post("studentJoinCourse", qs.stringify(user), {headers: {'Content-Type': 'application/x-www-form-urlencoded'}});
      if (res.flag === 'right') {
  ...
  mounted() {
    this.joinCourse = this.$store.state.student.joinCourse;
    this.getCourseByNum();
  }
}
</script>
...
.joinClass {
  ...
  z-index: 2;
  min-height: 160px;
  background: url('../../../static/course/1_png.png') no-repeat 50%;
