    ...
    <el-main>
      <div class="con">
        <div class="classOne" v-for="item in workList">
          ...
          <div class="foot">
            <button :id="item.homeworkNum" class="fi_bt" @click="doHomework($event)">做作业</button>
          </div>
        </div>
      </div>
    </el-main>
...
export default {
  ...
  methods: {
    doHomework(event) {
      this.$store.commit("setHomeworkNum", event.currentTarget.id)
      this.$router.push("/doHomework")
    },
