      ...
      <div class="choiceText">
        ...
        <div class="choiceItem">
          <div v-for="(item,index) in letterList">
            <p class="items" tabindex="0" :id="item" @click="changeAnswer($event)">
              <span class="options">
                {{ item }}
              </span>
              <span class="text">
      ...
      <div class="answerText">
        ...
        <div class="answerList">
          <div v-for="item in letterList" style="display: inline-block">
            <span class="c_item" v-if="answerList.indexOf(item) === -1">
              {{ item }}
            </span>
            <span class="c_itemFocus" v-else>
              {{ item }}
            </span>
          </div>
        ...
        <div class="answerList">
          ...
          <span class="a_item" v-for="item in rightAnswer">
            {{ item }}
          </span>
        </div>
        <div class="answerList">
...
export default {
  ...
  methods: {
    async submitAnswer() {
      ...
      let {data: res} = await this.$http.post("verifyMultiply", qs.stringify(answer), {headers: {'Content-Type': 'application/x-www-form-urlencoded'}});
      this.rightAnswer = res.answerList;
      /*提交一下做题情况*/
    ...
    changeAnswer(event) {
      let index = this.answerList.indexOf(event.currentTarget.id);
      if (index === -1) {
        this.answerList.push(event.currentTarget.id);
      } else {
        this.answerList.splice(index, 1);
      }
      for (let i = 0; i < this.letterList.length; i++) {
        if (this.answerList.indexOf(this.letterList[i]) !== -1) {
          document.getElementById(this.letterList[i]).className = 'itemsFocus';
        } else {
          document.getElementById(this.letterList[i]).className = 'items';
        }
      }
    }
  },
